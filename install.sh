...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/1622663850/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${red}无法获取 x-ui 版本，请确保您的网络可以访问 GitHub API${plain}"
            exit 1
        fi
        echo -e "获取到 x-ui 最新版本: ${tag_version}，开始安装..."
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/1622663850/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/1622663850/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
        echo -e "开始安装 x-ui $1"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/1622663850/3x-ui/main/x-ui.sh
